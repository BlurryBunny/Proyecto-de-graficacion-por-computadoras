...
//  Proyecciones
    ...
    cout<<"Se crearon todos los objetos"<<endl;
    /*-------------------------- Fish --------------------------*/
    ...
    arma::fmat transform_fishbowl;
    transform_fishbowl= Tr.S(0.1, 0.1, 0.1);
    ...
    do {
        ...
        for(unsigned int i=0; i<my_objs.size();i++){
            if(my_objs[i].get_objName()!="Fishbowl"){
                my_objs[i].move_and_paint(transform_fishbowl);
            }else{
                my_objs[i].move_and_paint(trans_size);
            }
        }
